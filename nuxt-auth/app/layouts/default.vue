<template>
  <div>
    <div>
      ...
      <Drawer v-model:visible="visible" header="Drawer" :pt="{
          content: { class: 'p-0' }
        }"  >
        <template #header>
    ...
    <div class="flex h-screen overflow-hidden">
      ...
      <div
        :class="[
          'bg-gray-800 text-white w-64 transition-transform duration-300 ease-in-out',
          isOpen ? 'translate-x-0' : '-translate-x-full',
          'fixed top-0 left-0 h-full z-30'
        ]"
      >
        <div class="p-4 font-bold">📊 대시보드</div>
      </div>
      ...
      >
        <div class="p-4">
          <button @click="isOpen = !isOpen" class="px-4 py-2 bg-blue-600 text-white rounded">
            {{ isOpen ? '닫기' : '열기' }} 사이드바
          </button>
          <div>
            <p class="mt-4">여기가 본문 콘텐츠입니다.</p>
